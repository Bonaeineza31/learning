...
export function App() {
  ...
  const handleLogin = (name, role, token) => {
    setIsLoggedIn(true)
    setUserName(name)
    setUserRole(role)
    setAuthToken(token)
  ...
  const loadContacts = async () => {
    try {
      ...
      if (!response.ok) throw new Error(result.message || 'Failed to load recent submissions.')
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setStatus({ loading: false, success: 'Your message has been sent successfully. We will get back to you soon.', error: '' })
      setForm(initialForm)
      setErrors({})
      await loadContacts()
    } catch (error) {
      setStatus({ loading: false, success: '', error: error.message || 'Failed to submit the form. Please try again.' })
    }
  }
  ...
  return (
    <>
      ...
      <main class="contact-page" id="home">
        <section class="contact-shell">
          ...
          <div class="contact-layout" id="contacts">
            <div class="contact-card">
              <form class="contact-form" onSubmit={handleSubmit} noValidate>
                <div class="field-row">
                  <label class="field" htmlFor="name">
                    <span>Name</span>
                    <input id="name" name="name" type="text" value={form.name} onInput={handleChange} placeholder="Your name" class={errors.name ? 'input error' : 'input'} />
                ...
                <div class="field-row two-col">
                  <label class="field" htmlFor="email">
                    <span>Email</span>
                    <input id="email" name="email" type="email" value={form.email} onInput={handleChange} placeholder="you@example.com" class={errors.email ? 'input error' : 'input'} />
                    {errors.email && <small class="error-text">{errors.email}</small>}
                  </label>
                  <label class="field" htmlFor="phone">
                    <span>Phone</span>
                    <input id="phone" name="phone" type="tel" value={form.phone} onInput={handleChange} placeholder="+250712345678" class={errors.phone ? 'input error' : 'input'} />
                ...
                <label class="field" htmlFor="message">
                  <span>Message</span>
                  <textarea id="message" name="message" rows="6" value={form.message} onInput={handleChange} placeholder="Tell us about your requirements" class={errors.message ? 'textarea error' : 'textarea'} />
            ...
            <aside class="map-card" aria-label="AC Mobility location map">
              ...
              <div class="map-wrap">
                <iframe title="AC Mobility location map" src="https://www.google.com/maps?q=AC%20Mobility&output=embed" loading="lazy" allowFullScreen />
              </div>
              ...
              <div class="records-card">
                ...
                {!loadingContacts && !contactsError && contacts.length === 0 && <p class="records-status">No submissions yet.</p>}
      ...
      {isLoggedIn && <Products token={authToken} userRole={userRole} />}
    </>
  )
}
